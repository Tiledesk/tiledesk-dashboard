...
/** When a guest closes the conversation, actor.id matches actionObj.lead.lead_id. */
export function resolveClosedByLeadContext(activity: ActivityRecord): { fullname: string; contactId: string } | null {
  ...
  const lead = activity.actionObj?.['lead'] as Record<string, unknown> | undefined;
  if (!lead) {
    return null;
  }

  const leadId = str(lead['lead_id'] || lead['leadId']).trim();
  if (!leadId || leadId !== actorId) {
    return null;
  }
