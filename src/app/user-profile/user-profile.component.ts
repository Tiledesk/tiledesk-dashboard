  ...
  updateCurrentUserFirstnameLastname() {
    ...
    this.usersService.updateCurrentUserLastnameFirstname(this.userFirstname, this.userLastname, (response) => {
      ...
      if (response === 'success') {
        ...
        // =========== NOTIFY SUCCESS===========
        this.notify.showWidgetStyleUpdateNotification(this.translate.instant('UserProfile.YourProfileHasBeenUploaded'), 2, 'done');

      } else if (response === 'error') {
        this.SHOW_CIRCULAR_SPINNER = false;
        this.UPDATE_USER_ERROR = true;
        // =========== NOTIFY ERROR ===========
        this.notify.showWidgetStyleUpdateNotification(this.translate.instant('UserProfile.AnErrorHasOccurred'), 4, 'report_problem')
      }
    });
  }
