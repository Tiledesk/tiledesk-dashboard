  ...
  updateCurrentUserFirstnameLastname() {
    ...
    this.usersService.updateCurrentUserLastnameFirstname(this.userFirstname, this.userLastname, (response) => {
      ...
      if (response === 'success') {
        ...
        this.trackUpdateProfileName(response)
        this.getLoggedUser()
        ...
        // =========== NOTIFY SUCCESS===========
        this.notify.showNotification(this.translate.instant('UserProfile.YourProfileHasBeenUploaded'), 2, 'done');

      } else if (response === 'error') {
        this.SHOW_CIRCULAR_SPINNER = false;
        this.UPDATE_USER_ERROR = true;
        // =========== NOTIFY ERROR ===========
        this.notify.showNotification(this.translate.instant('UserProfile.AnErrorHasOccurred'), 4, 'report_problem')
      }
    });
  }
