...
export class Home2ProjectAttributesVmService {
  getVm(projectAttributes: any): Home2ProjectAttributesVm {
    ...
    if (projectAttributes?.userPreferences) {
      vm.onboarding = this.mapUserPreferencesToOnboardingVm(projectAttributes.userPreferences);
    }

    return vm;
  }

  mapUserPreferencesToOnboardingVm(userPreferences: any): Home2OnboardingVm {
    ...
    const usecaseVm = this.resolveUsecaseVm(solution, solution_channel, use_case);
    return {
      ...defaults,
      ...usecaseVm,
  ...
  private getDefaultOnboarding(): Omit<Home2OnboardingVm, 'displayKbHeroSection' | 'solution' | 'solution_channel' | 'use_case'> {
    return {
      child_list_order: [
        { pos: 1, type: 'child1' },
        { pos: 2, type: 'child2' },
  ...
  private resolveUsecaseVm(
    solution: string | undefined,
    solution_channel: string | undefined,
    use_case: string | undefined
  ): Partial<Omit<Home2OnboardingVm, 'displayKbHeroSection' | 'solution' | 'solution_channel' | 'use_case'>> {
    // NOTE: We only migrate the most common branches now to minimize risk.
    // Unhandled combinations fall back to defaults.
