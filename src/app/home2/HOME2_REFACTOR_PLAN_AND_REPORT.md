...
- **Status**: completed
- **Changes**:
  - Avvio Step 3: estratti i primi side-effect fuori da `Home2Component`:
    - `Home2StorageService` per key `dshbrd----hasclosedpopup` e flag `dshbrd----{projectId}` (trial ended emitted)
    - `Home2TrackingService` per `analytics.page/track/identify/group` (inizialmente applicato a `pageHome()` in `ngAfterViewInit`)
  - `Home2Component` non usa più direttamente `localStorage` (resta solo una riga commentata legacy).
  - Estratte tutte le chiamate dirette a `window['analytics']` dal component:
    - `trackTrialEnded()` ora delega a `Home2TrackingService.trackTrialEnded()`
    - `trackGroup()` ora delega a `Home2TrackingService.groupProject()`
    - `trackUserAction()` ora delega a `Home2TrackingService.track/identifyUser/groupProject()`
    - `getLoggedUser()` ora delega a `Home2TrackingService.identifyUser()`
  - Estratte le navigazioni (Router + link esterni) in `Home2NavigationService`:
    - sostituiti `router.navigate(...)` con `home2Nav.goTo*()`
    - sostituiti `window.open(...)` con `home2Nav.openExternal/openMailTo()`
  - Estratte le modali (SweetAlert2 + NotifyService) in `Home2ModalService`:
    - rimosso `require('sweetalert2')` dal component e sostituito `Swal.fire(...)` con `home2Modal.fire(...)`
    - sostituiti `notify._displayContactUsModal(...)` e `notify.presentModalOnlyOwnerCanManageTheAccountPlan(...)` con `home2Modal.*`
- **Verifiche eseguite**:
  - Build applicazione (`ng build`) OK.
- **Esito**:
  - OK (side-effects principali isolati; nel component restano solo 2 righe commentate legacy).

#### Step 4
 **Status**: completed
- **Changes**:
  - Consolidamento wiring:
    - rimosso `AuthService` dal component; spostato `projectProfile(...)` in `Home2ProjectProfileService`
    - spostato `hasChangedProjectFroList$` dentro `Home2Facade` (`hasChangedProjectFromList$`) e aggiornato il component
  - Hardening:
    - rimosse dipendenze non più usate dal component (`Router`, `NotifyService`, `sweetalert`)
    - marcati metodi legacy `getDashlet()` e `getOnbordingPreferences()` come non più usati (sostituiti dal VM)
  - Rollback documentato:
    - per tornare al vecchio wiring basta ripristinare in `src/app/app.routing.ts` la rotta:
      - da `loadChildren: () => import('app/home2/home2.module').then(m => m.Home2Module)`
      - a `loadChildren: () => import('app/home/home.module').then(m => m.HomeModule)`
- **Verifiche eseguite**:
  - Build applicazione (`ng build`) OK.
- **Esito**:
  - OK
