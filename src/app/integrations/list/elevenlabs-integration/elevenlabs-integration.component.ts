...
@Component({
  selector: 'elevenlabs-integration',
  templateUrl: './elevenlabs-integration.component.html',
  styleUrls: ['./elevenlabs-integration.component.scss']
})
export class ElevenlabsIntegrationComponent implements OnInit {
   ...
   ngOnInit(): void {
    this.logger.log("[INT-elevenlabs] integration ", this.integration)
     this.translateparams = { intname: 'ElevenLabs' };
  ...
  checkKey() {
    return new Promise((resolve) => {
      ...
      this.integrationService
        .checkElevenLabsKeyValidity(url, this.integration.value.apikey)
        .subscribe(
          (resp) => {
            this.logger.log("[INT-elevenlabs] Key verification resp: ", resp);
            this.isVerified = true;
            resolve(true);
          },
          (error) => {
            this.logger.error("[INT-elevenlabs] Key verification failed: ", error);
