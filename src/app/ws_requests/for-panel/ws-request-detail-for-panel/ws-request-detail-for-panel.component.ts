      ...
      .subscribe(status => {
        ...
        if (status.role !== 'owner' && status.role !== 'admin' && status.role !== 'agent') {
          if (status.matchedPermissions.includes(PERMISSIONS.REQUEST_CLOSE)) {
            this.logger.log('[REQUEST-DTLS-X-PANEL] PERMISSION_TO_ARCHIVE_REQUEST', PERMISSIONS.REQUEST_CLOSE)
            
            this.PERMISSION_TO_ARCHIVE_REQUEST = true
            this.logger.log('[REQUEST-DTLS-X-PANEL] - PERMISSION_TO_ARCHIVE_REQUEST 1 ', this.PERMISSION_TO_ARCHIVE_REQUEST);
          } else {
            this.PERMISSION_TO_ARCHIVE_REQUEST = false
            this.logger.log('[REQUEST-DTLS-X-PANEL] - PERMISSION_TO_ARCHIVE_REQUEST 2', this.PERMISSION_TO_ARCHIVE_REQUEST);
          }
        } else {
          this.PERMISSION_TO_ARCHIVE_REQUEST = true
          this.logger.log('[REQUEST-DTLS-X-PANEL] - Project user has a default role 3', status.role, 'PERMISSION_TO_ARCHIVE_REQUEST ', this.PERMISSION_TO_ARCHIVE_REQUEST);
        }

          // PERMISSION_TO_JOIN_REQUEST
        if (status.role !== 'owner' && status.role !== 'admin' && status.role !== 'agent') {
          if (status.matchedPermissions.includes(PERMISSIONS.REQUEST_JOIN)) {
            this.logger.log('[REQUEST-DTLS-X-PANEL] PERMISSION_TO_JOIN_REQUEST', PERMISSIONS.REQUEST_JOIN)
            
            this.PERMISSION_TO_JOIN_REQUEST = true
            this.logger.log('[REQUEST-DTLS-X-PANEL] - PERMISSION_TO_JOIN_REQUEST 1 ', this.PERMISSION_TO_JOIN_REQUEST);
          } else {
            this.PERMISSION_TO_JOIN_REQUEST = false
            this.logger.log('[REQUEST-DTLS-X-PANEL] - PERMISSION_TO_JOIN_REQUEST 2', this.PERMISSION_TO_JOIN_REQUEST);
          }
        } else {
          this.PERMISSION_TO_JOIN_REQUEST = true
          this.logger.log('[REQUEST-DTLS-X-PANEL] - Project user has a default role 3', status.role, 'PERMISSION_TO_JOIN_REQUEST ', this.PERMISSION_TO_JOIN_REQUEST);
        }
